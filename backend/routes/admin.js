...
router.get('/admin/stats', (req, res, next) => {
  try {
    ...
    const flashcardCount = db.prepare('SELECT COUNT(*) AS count FROM flashcards').get().count
    ...
    const totalXp = db.prepare('SELECT COALESCE(SUM(total_xp), 0) AS total FROM user_stats').get().total
    const adminCount = db.prepare('SELECT COUNT(*) AS count FROM users WHERE is_admin = 1').get().count
    const masteryCount = db.prepare('SELECT COUNT(*) AS count FROM mastery_scores').get().count
...
router.get('/admin/tracks', (req, res, next) => {
  try {
    const tracks = db.prepare(`
      SELECT t.*,
             (SELECT COUNT(*) FROM track_courses tc WHERE tc.track_id = t.id) AS course_count,
             (SELECT COUNT(*) FROM track_courses tc JOIN courses c ON c.id = tc.course_id WHERE tc.track_id = t.id AND c.is_deleted = 0 AND c.is_archived = 0) AS active_course_count
      FROM tracks t ORDER BY t.id ASC
    `).all()
    for (const track of tracks) {
      track.courses = db.prepare(`
...
router.post('/admin/tracks', (req, res, next) => {
  try {
    const { name, slug, description, language, color } = req.body
    if (!name || !slug) return res.status(400).json({ error: 'Name and slug are required' })
    const existing = db.prepare('SELECT id FROM tracks WHERE slug = ?').get(slug)
    if (existing) return res.status(409).json({ error: 'Slug already exists' })
    const result = db.prepare('INSERT INTO tracks (name, slug, description, language, color) VALUES (?, ?, ?, ?, ?)').run(name, slug, description || '', language || 'Python', color || '#03ef62')
...
router.patch('/admin/tracks/:id', (req, res, next) => {
  try {
...
router.delete('/admin/tracks/:id', (req, res, next) => {
  try {
    const trackId = Number(req.params.id)
    const track = db.prepare('SELECT id FROM tracks WHERE id = ?').get(trackId)
    if (!track) return res.status(404).json({ error: 'Track not found' })
    const hasCourses = db.prepare('SELECT COUNT(*) AS count FROM track_courses tc JOIN courses c ON c.id = tc.course_id WHERE tc.track_id = ? AND c.is_deleted = 0').get(trackId).count
...
router.post('/admin/tracks/:id/archive', (req, res, next) => {
  try {
    const trackId = Number(req.params.id)
    const track = db.prepare('SELECT id FROM tracks WHERE id = ?').get(trackId)
    if (!track) return res.status(404).json({ error: 'Track not found' })
    db.prepare('UPDATE tracks SET is_archived = 1 WHERE id = ?').run(trackId)
    log('archive_track', { track_id: trackId })
    res.json({ success: true })
  } catch (err) { next(err) }
})

router.post('/admin/tracks/:id/restore', (req, res, next) => {
  try {
    const trackId = Number(req.params.id)
    const track = db.prepare('SELECT id FROM tracks WHERE id = ?').get(trackId)
    if (!track) return res.status(404).json({ error: 'Track not found' })
    db.prepare('UPDATE tracks SET is_archived = 0, is_deleted = 0 WHERE id = ?').run(trackId)
...
router.get('/admin/courses', (req, res, next) => {
  try {
    ...
    if (status === 'deleted') { where += ' AND c.is_deleted = 1' }
    else if (status === 'archived') { where += ' AND c.is_archived = 1' }
    else if (status === 'active') { where += ' AND c.is_deleted = 0 AND c.is_archived = 0' }
    ...
    for (const c of courses) {
      ...
      c.has_exercises = c.has_exercises > 0 ? 1 : 0
    }
...
router.put('/admin/courses/:id', (req, res, next) => {
  try {
...
router.delete('/admin/courses/:id', (req, res, next) => {
  try {
    const courseId = Number(req.params.id)
    const course = db.prepare('SELECT id, slug FROM courses WHERE id = ?').get(courseId)
    if (!course) return res.status(404).json({ error: 'Course not found' })
    const result = db.transaction(() => {
      const attempts = db.prepare('DELETE FROM exercise_attempts WHERE course_id = ?').run(courseId).changes
      const mastery = db.prepare('DELETE FROM mastery_scores WHERE course_id = ?').run(courseId).changes
      const uc = db.prepare('DELETE FROM user_courses WHERE course_id = ?').run(courseId).changes
      const srq = db.prepare('DELETE FROM spaced_repetition_queue WHERE flashcard_id IN (SELECT id FROM flashcards WHERE course_id = ?)').run(courseId).changes
      const ufp = db.prepare('DELETE FROM user_flashcard_progress WHERE flashcard_id IN (SELECT id FROM flashcards WHERE course_id = ?)').run(courseId).changes
      const fc = db.prepare('DELETE FROM flashcards WHERE course_id = ?').run(courseId).changes
      const qq = db.prepare('DELETE FROM quiz_questions WHERE course_id = ?').run(courseId).changes
      const concepts = db.prepare('DELETE FROM concepts WHERE course_id = ?').run(courseId).changes
      const tc = db.prepare('DELETE FROM track_courses WHERE course_id = ?').run(courseId).changes
...
router.post('/admin/courses/:id/clear-exercises', (req, res, next) => {
  try {
    const courseId = Number(req.params.id)
    db.transaction(() => {
      ...
      db.prepare(`UPDATE user_courses SET status = 'Not Started' WHERE course_id = ?`).run(courseId)
    })()
...
router.post('/admin/courses/reorder', (req, res, next) => {
  try {
    ...
    const reorder = db.transaction((trackId, orderedIds) => {
...
router.post('/admin/courses/:id/move-track', (req, res, next) => {
  try {
    const courseId = Number(req.params.id)
    ...
    db.prepare('UPDATE track_courses SET track_id = ?, order_in_track = ? WHERE track_id = ? AND course_id = ?').run(to_track_id, order_in_track || 1, from_track_id, courseId)
    log('move_track', { course_id: courseId, from_track: from_track_id, to_track: to_track_id })
    res.json({ success: true })
  } catch (err) { next(err) }
})

router.post('/admin/courses/:id/add-to-track', (req, res, next) => {
  try {
    const courseId = Number(req.params.id)
    const { track_id, order_in_track } = req.body
    if (!track_id) return res.status(400).json({ error: 'track_id required' })
    db.prepare('INSERT OR IGNORE INTO track_courses (track_id, course_id, order_in_track) VALUES (?, ?, ?)').run(track_id, courseId, order_in_track || 1)
    log('add_course_to_track', { course_id: courseId, track_id })
    res.json({ success: true })
  } catch (err) { next(err) }
})

router.post('/admin/courses/:id/remove-from-track', (req, res, next) => {
  try {
    const courseId = Number(req.params.id)
    const { track_id } = req.body
    if (!track_id) return res.status(400).json({ error: 'track_id required' })
    db.prepare('DELETE FROM track_courses WHERE track_id = ? AND course_id = ?').run(track_id, courseId)
...
router.get('/admin/courses/:id/exercises/summary', (req, res, next) => {
  try {
    const courseId = Number(req.params.id)
    const course = db.prepare('SELECT c.*, (SELECT tc.track_id FROM track_courses tc WHERE tc.course_id = c.id LIMIT 1) AS track_id FROM courses c WHERE id = ?').get(courseId)
    if (!course) return res.status(404).json({ error: 'Course not found' })
    const concepts = db.prepare('SELECT COUNT(*) AS count FROM concepts WHERE course_id = ?').get(courseId).count
    const flashcards = db.prepare('SELECT COUNT(*) AS count FROM flashcards WHERE course_id = ?').get(courseId).count
    const quiz_questions = db.prepare('SELECT COUNT(*) AS count FROM quiz_questions WHERE course_id = ?').get(courseId).count
    const total_attempts = db.prepare('SELECT COUNT(*) AS count FROM exercise_attempts WHERE course_id = ?').get(courseId).count
    const unique_students = db.prepare('SELECT COUNT(DISTINCT user_id) AS count FROM exercise_attempts WHERE course_id = ?').get(courseId).count
...
router.post('/admin/courses/:id/exercises/reimport', (req, res, next) => {
  try {
    const courseId = Number(req.params.id)
    const course = db.prepare('SELECT slug FROM courses WHERE id = ?').get(courseId)
    if (!course) return res.status(404).json({ error: 'Course not found' })
...
router.delete('/admin/courses/:id/exercises/type/:exerciseType', (req, res, next) => {
  try {
    const courseId = Number(req.params.id)
    const { exerciseType } = req.params
    let count = 0
    if (exerciseType === 'quiz') {
      count = db.prepare('DELETE FROM quiz_questions WHERE course_id = ?').run(courseId).changes
      db.prepare("DELETE FROM exercise_attempts WHERE course_id = ? AND exercise_type = 'quiz'").run(courseId)
    } else if (exerciseType === 'flashcard') {
      count = db.prepare('DELETE FROM flashcards WHERE course_id = ?').run(courseId).changes
      db.prepare('DELETE FROM user_flashcard_progress WHERE flashcard_id IN (SELECT id FROM flashcards WHERE course_id = ?)').run(courseId)
      db.prepare('DELETE FROM spaced_repetition_queue WHERE flashcard_id IN (SELECT id FROM flashcards WHERE course_id = ?)').run(courseId)
      db.prepare("DELETE FROM exercise_attempts WHERE course_id = ? AND exercise_type = 'flashcard'").run(courseId)
    } else {
      db.prepare('DELETE FROM exercise_attempts WHERE course_id = ? AND exercise_type = ?').run(courseId, exerciseType)
      count = 0
    }
...
router.get('/admin/courses/:id/reset-stats', (req, res, next) => {
  try {
    const courseId = Number(req.params.id)
    const studentCount = db.prepare('SELECT COUNT(*) AS count FROM user_courses WHERE course_id = ?').get(courseId).count
    const attemptCount = db.prepare('SELECT COUNT(*) AS count FROM exercise_attempts WHERE course_id = ?').get(courseId).count
    res.json({ student_count: studentCount, attempt_count: attemptCount })
  } catch (err) { next(err) }
})

router.get('/admin/tracks/:id/reset-stats', (req, res, next) => {
  try {
    const trackId = Number(req.params.id)
    const courses = db.prepare('SELECT course_id AS id FROM track_courses WHERE track_id = ?').all(trackId)
    let totalAttempts = 0
    for (const c of courses) {
      totalAttempts += db.prepare('SELECT COUNT(*) AS count FROM exercise_attempts WHERE course_id = ?').get(c.id).count
    }
...
router.post('/admin/reset/course/:id', (req, res, next) => {
  try {
...
router.post('/admin/reset/track/:id', (req, res, next) => {
  try {
    if (!req.body.confirm) return res.status(400).json({ error: 'confirm: true is required' })
    const trackId = Number(req.params.id)
    const courses = db.prepare('SELECT course_id AS id FROM track_courses WHERE track_id = ?').all(trackId)
    ...
    db.transaction(() => {
      for (const c of courses) {
        const attempts = db.prepare('DELETE FROM exercise_attempts WHERE course_id = ?').run(c.id).changes
        const mastery = db.prepare('DELETE FROM mastery_scores WHERE course_id = ?').run(c.id).changes
        db.prepare("UPDATE user_courses SET status = 'Not Started' WHERE course_id = ?").run(c.id)
...
router.post('/admin/reset/all', (req, res, next) => {
  try {
    ...
    db.transaction(() => {
      ...
      db.prepare('DELETE FROM mastery_scores').run()
      db.prepare('DELETE FROM user_flashcard_progress').run()
      db.prepare('DELETE FROM spaced_repetition_queue').run()
      db.prepare("UPDATE user_courses SET status = 'Not Started'").run()
      db.prepare('UPDATE user_stats SET total_xp=0, level=\'Beginner\', current_streak=0, longest_streak=0, last_active_date=NULL').run()
    })()
...
router.patch('/admin/users/:id', (req, res, next) => {
  try {
    ...
    const updates = []
    const values = []
    if (req.body.is_admin !== undefined) { updates.push('is_admin = ?'); values.push(req.body.is_admin ? 1 : 0) }
...
router.post('/admin/users', (req, res, next) => {
  try {
    ...
    const result = db.prepare('INSERT INTO users (username, password_hash, salt, is_admin) VALUES (?, ?, ?, ?)').run(username.trim(), hash, salt, is_admin ? 1 : 0)
    const userId = result.lastInsertRowid
    db.prepare('INSERT INTO user_stats (user_id) VALUES (?)').run(userId)
    log('create_user', { user_id: userId })
    res.json({ success: true, user: { id: userId, username: username.trim(), is_admin: is_admin ? 1 : 0 } })
  } catch (err) { next(err) }
})
...
router.delete('/admin/users/:id', (req, res, next) => {
  try {
    ...
    db.transaction(() => {
      ...
      db.prepare('DELETE FROM user_courses WHERE user_id = ?').run(targetId)
      db.prepare('DELETE FROM user_flashcard_progress WHERE user_id = ?').run(targetId)
      db.prepare('DELETE FROM spaced_repetition_queue WHERE user_id = ?').run(targetId)
      db.prepare('DELETE FROM exercise_attempts WHERE user_id = ?').run(targetId)
      db.prepare('DELETE FROM mastery_scores WHERE user_id = ?').run(targetId)
      db.prepare('DELETE FROM user_stats WHERE user_id = ?').run(targetId)
...
router.post('/admin/users/:id/toggle-admin', (req, res, next) => {
  try {
    const targetId = Number(req.params.id)
...
router.post('/admin/users/:id/reset-progress', (req, res, next) => {
  try {
    if (!req.body.confirm) return res.status(400).json({ error: 'confirm: true is required' })
    const userId = Number(req.params.id)
    db.transaction(() => {
      ...
      db.prepare("UPDATE user_courses SET status = 'Not Started' WHERE user_id = ?").run(userId)
      db.prepare('UPDATE user_stats SET total_xp=0, level=\'Beginner\', current_streak=0, longest_streak=0 WHERE user_id = ?').run(userId)
    })()
...
router.get('/admin/users/:id/progress', (req, res, next) => {
  try {
    const userId = Number(req.params.id)
...
router.get('/admin/system/stats', (req, res, next) => {
  try {
    const total_users = db.prepare('SELECT COUNT(*) AS count FROM users').get().count
    const total_courses = db.prepare('SELECT COUNT(*) AS count FROM courses').get().count
    const total_tracks = db.prepare('SELECT COUNT(*) AS count FROM tracks').get().count
    const total_attempts = db.prepare('SELECT COUNT(*) AS count FROM exercise_attempts').get().count
    const total_concepts = db.prepare('SELECT COUNT(*) AS count FROM concepts').get().count
    const total_flashcards = db.prepare('SELECT COUNT(*) AS count FROM flashcards').get().count
    const total_quiz_questions = db.prepare('SELECT COUNT(*) AS count FROM quiz_questions').get().count
    let db_size_mb = 0
...
router.get('/admin/system/logs', (req, res, next) => {
  try {
...
router.post('/admin/system/reimport-all', (req, res, next) => {
  try {
    const result = importJsonExercises()
...
router.get('/admin/system/config', (req, res, next) => {
  try {
...
router.post('/admin/reset/nuclear', (req, res, next) => {
  try {
    ...
    db.transaction(() => {
      ...
      db.prepare('DELETE FROM spaced_repetition_queue').run()
      db.prepare('DELETE FROM user_flashcard_progress').run()
      db.prepare('DELETE FROM user_courses').run()
      db.prepare('DELETE FROM user_tracks').run()
      ...
      db.prepare('DELETE FROM track_courses').run()
      db.prepare('DELETE FROM courses').run()
      db.prepare('DELETE FROM tracks').run()
      db.prepare('DELETE FROM deleted_questions').run()
    })()
