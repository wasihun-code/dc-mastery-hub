    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black p-4 select-none">
      ...
      <div className="relative z-10 w-full max-w-md bg-[#15161e]/90 border border-[var(--border)] rounded-3xl p-8 shadow-2xl backdrop-blur-md animate-in fade-in zoom-in-95 duration-200">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="mt-1 text-3xl font-black text-white italic tracking-tighter uppercase text-center">SECURE LOGIN</h1>
        </div>

        {error && (
          <div className="mb-6 flex gap-2.5 items-start rounded-xl border border-red-500/30 bg-red-950/20 p-4 text-red-400 text-sm animate-in slide-in-from-top-2">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-8 rounded-2xl bg-[var(--accent-green)] py-4 text-base font-bold text-black shadow-lg hover:shadow-xl transition-all hover:scale-[1.02] active:scale-[0.98] hover:bg-[var(--accent-green-bright)] disabled:opacity-50 disabled:pointer-events-none flex items-center justify-center gap-2 font-black tracking-wide cursor-pointer"
          >
            {loading ? 'AUTHENTICATING...' : (
              <>
                LOG IN <ArrowRight size={18} strokeWidth={2.5} />
              </>
            )}
          </button>
        </form>
