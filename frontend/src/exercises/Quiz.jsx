...
export default function Quiz() {
  ...
  const [sessionShown] = useState(new Set())
  ...
  const fetchQuestions = async () => {
    try {
      setLoading(true)
      const excludeIds = Array.from(sessionShown).join(',')
      const url = `/api/courses/${courseSlug}/quiz-questions?count=20${excludeIds ? '&exclude_ids=' + excludeIds : ''}`
  ...
  const handleNext = () => {
    if (currentIndex < questions.length - 1) {
      ...
      while (nextIndex < questions.length && sessionShown.has(questions[nextIndex].id)) {
        nextIndex++
      }
      if (nextIndex < questions.length) {
        ...
        // We've exhausted this batch, but let's just finish the quiz for now
        // To be fully robust, we would fetch more here if we hadn't hit our target.
            ...
            <div className="mt-12 flex flex-col gap-3">
              <button
                onClick={() => {
                  ...
                  setStartTime(Date.now())
                  startTimer()
                  fetchQuestions() // Refetch to get new batch excluding shown
                }}
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-8 shadow-sm">
          <h3 className="text-2xl font-bold leading-tight text-[var(--text-primary)]">
            {renderWithCode(question.question_text)}
          </h3>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-lg font-medium">{renderWithCode(option.text)}</span>
                  </div>
            ...
            <div className="mt-8 animate-in fade-in slide-in-from-top-4 duration-300">
              <div className={`rounded-xl p-6 ${selectedOption === question.correct_option ? 'bg-[rgba(3,239,98,0.05)] border border-[var(--accent-green)]' : 'bg-[rgba(239,68,68,0.05)] border border-[var(--accent-red)]'}`}>
                ...
                <p className="text-[var(--text-muted)] leading-relaxed">
                  {renderWithCode(question.explanation)}
                </p>
              </div>
